...
public class AdvCommand {
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("advjs").requires(sourceStack -> sourceStack.hasPermission(2))
            ...
            .then(Commands.literal("nether").executes(context -> generate(context.getSource(), AdvJS.NETHER, NETHER)))
            .then(Commands.literal("husbandry").executes(context -> generate(context.getSource(), AdvJS.HUSBANDRY, HUSBANDRY)))
            .then(Commands.literal("end").executes(context -> generate(context.getSource(), AdvJS.END, END)))
    ...
    private static int generate(CommandSourceStack sourceStack, Path file, String text) {
        try {
            Files.writeString(file, text);
            sourceStack.sendSuccess(() -> Component.translatable("advjs.command.success", file.getFileName()), false);
            return 1;
        } catch (Exception e) {
            throw new CommandRuntimeException(Component.translatable("advjs.command.failed", file.getFileName()));
        }
    }
